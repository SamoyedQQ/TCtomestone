...
```python
# rankings.duration 「通常」已扣 downtime，但部分情境會回傳 ≈ raw fight time（沒扣），
# 偵測訊號：rd > raw - 50_000。已知影響：zone 62 嵌入的絕境戰、FRU(1079) zone 65。
if fight_id in rankings_duration:
    rd = rankings_duration[fight_id]
    if rd > raw_ms - 50_000:
        # rd 沒扣 downtime → 改用 totalTime - damageDowntime；缺值才退回估算表
        effective_ms = total_time_ms - (damage_downtime_ms or estimate)
    else:
        effective_ms = rd                              # 首選：與網頁完全一致
else:
    effective_ms = total_time_ms - damage_downtime_ms  # 完全沒 rankings 時的 fallback
...
_ENCOUNTER_DOWNTIME_ESTIMATE = {
    1077: 275_000,  # TOP：實測 ≈ 275s
    1079: 287_500,  # FRU：實測 ≈ 287.5s（zone 65 DETAIL_QUERY 通常有真實值，此為 fallback）
}
```
...
**FRU（1079）在 zone 65 的已知問題（2026-06 發現）：**  
FRU 的 `rankings.duration` 與 `raw_ms` 僅差約 1 秒（FFLogs 沒在裡面扣 phase transition / BOSS 上天時段），但實測 `damageDowntime ≈ 287.5s`（佔全程 25%）。  
**症狀**：同一場通關，**有 ranking 的紀錄** rDPS 約 19–20k（用 rd ≈ 1123s 為分母，偏低）；**沒 ranking 的紀錄** rDPS 約 27–28k（fallback 到 totalTime − damageDowntime ≈ 837s，正確）。  
**通用偵測訊號**：`rd > raw_ms - 50_000` 即代表 `rankings.duration` 沒扣 downtime。命中時優先用 `totalTime - damageDowntime`（FRU zone 65 / TOP zone 59 都有真實值），缺值才退回估算表（zone 62 嵌入場次走這條）。同一機制同時處理 zone 62 嵌入場次與 FRU 主 zone 場次。
