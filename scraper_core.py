...
class Scraper:
    ...
    def run(self):
        ...
        r = self._do_scan(self.start_from_ms, self.end_from_ms, seen_keys, seen_clear_sigs, bests)
        bests.save()
            ...
            for page in range(1, self._max_pages + 1):
                ...
                if _preloaded is not None:
                    ...
                    data = self._gql(SCAN_QUERY, {
                        "page": page, "startTime": batch_start, "endTime": end_time
                    })
                    if data is None:
                        break
                ...
                if res.pts_used >= self._point_limit:
                    self.on_log(f"積分上限 {res.pts_used}/{self._point_limit}pt，停止。")
                    batch_done = True
                    break
