...
| **P4/P5 轉換點全滅** | **= 80（與通關相同！）** | ~0%（BP 正好到閾值） |
| **通關** | **= 80（精確值）** | ~0% |

> 資料來源：`ucob_calibration.json`（report `m29TjCJwxaWqbLDt`）

**關鍵結論**：
- `fightPercentage == 80` 對應通關 **或** P4→P5 轉換點即時全滅，兩者無法僅憑 fp 區分
- duration ≥ 13min（780,000 ms）為必要的安全網：P4/P5 轉換全滅約在 10 分鐘，真實通關需完成整個 Golden Bahamut 段落，至少 13 分鐘（已知最短真實通關 ≈ 957 秒）
- 已知誤判案例：report `Pt8qYbmLydnMkraF` fight 23，時長 619 秒，fp=80，為 P4/P5 轉換全滅
...
if fight.get("encounterID") == 1073:
    return (
        ...
        and (fight["endTime"] - fight["startTime"]) >= 780_000  # 13 分鐘：排除 P4/P5 轉換點全滅
    )
```
